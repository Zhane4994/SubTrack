...
function writeAiUsageCount(value) {
  try {
    localStorage.setItem(getAiUsageStorageKey(), String(value));
  } catch {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderAiBudgetToolState(statusMessage = "") {
    ...
    const used = readAiUsageCount();

    aiBudgetUsageText.textContent = `Usage: ${used} / ${limit}`;
    ...
    runAiBudgetBtn.disabled = remaining <= 0;
    const planLabel = planType === "lifetime" ? "Lifetime" : "Premium";
    aiBudgetStatusText.textContent =
      statusMessage || `${planLabel} AI uses remaining: ${remaining} of ${limit}.`;
  }
